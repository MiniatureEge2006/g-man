...
| overlay | `!overlay [width] [height] [x] [y] [shortest] [ashortest]` | width and height: `50 to 1240`<br>x and y: `any number`<br> shortest: `0 or 1` | Overlays a video on top of another video. Will use the first video as the video to be overlayed and the previous one to overlay to the first video. By default the values are 480 for `[width]`, auto for `[height]`, 0 for `[x]` and `[y]`, false for `[shortest]`, and false for `[ashortest]`. Has similar functionality to greenscreen and scale.<br><br>`[width]` and `[height]` are to resize the video being overlayed onto the other video.<br><br>`[x]` and `[y]` are to overlay the video to at those coordinates of the other video.<br><br>`[shortest]` and `[ashortest]` are boolean values for both video and audio streams where if you specify false, the overlayed video will continue when the other video ends. If you specify true, the overlayed video will end when the original video duration ends. | `!overlay`<br><br>`!overlay 100 auto 50 30`<br><br>`!overlay 480 auto 0 0 true` |
