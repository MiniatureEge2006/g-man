...
| fade | `!fade [transition] [duration] [offset] [expr]` | duration: `0 to 60` | Advanced command that uses the xfade filter to fade to another video. A list of fade transitions can be found at https://trac.ffmpeg.org/wiki/Xfade#Gallery<br><br>The expression argument can only be used on the `custom` transition. | `!fade`<br><br>`!fade circlecrop 5 5` |
...
| overlay | `!overlay [width] [height] [x] [y] [shortest] [ashortest]` | width and height: `50 to 1240`<br>x and y: `any number`<br> shortest and ashortest: `true or false` | Overlays a video on top of another video. Will use the first video as the video to be overlayed and the previous one to overlay to the first video. By default the values are 480 for `[width]`, auto for `[height]`, 0 for `[x]` and `[y]`, false for `[shortest]`, and false for `[ashortest]`. Has similar functionality to greenscreen and scale.<br><br>`[width]` and `[height]` are to resize the video being overlayed onto the other video.<br><br>`[x]` and `[y]` are to overlay the video to at those coordinates of the other video.<br><br>`[shortest]` and `[ashortest]` are boolean values for both video and audio streams where if you specify false, the overlayed video will continue when the other video ends. If you specify true, the overlayed video will end when the original video duration ends. | `!overlay`<br><br>`!overlay 100 auto 50 30`<br><br>`!overlay 480 auto 0 0 true` |
...
| vid2img | `!vid2img` | | Converts a video to an image. | `!vid2img` |
| mp3 | `!mp3` | | Converts the video to an mp3. Also useful for downloading audio from websites like SoundCloud. | `!mp3`|
...
| Command | Format | Min/Max Values | Description | Examples |
| --- | --- | --- | --- | --- |
| reload | `!reload [cog]` | | Reloads a cog, or all cogs by just typing the command. Mostly useful to reload a cog to see changes. | `!reload`<br><br>`!reload cogs.filter` |
