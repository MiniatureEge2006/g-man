...
class Exif(commands.Cog):
    ...
    @commands.hybrid_command(
        name="exif",
        description="Use FFprobe to extract exif metadata from media.",
        aliases=["ffprobe"],
    )
    @app_commands.describe(
        url="Input URL to extract metadata from.",
        attachment="Media attachment to extract metadata from.",
    )
        ...
        try:
            ...
            if metadata:
                ...
                if "image" in mime_type:
                    ...
                    thumbnail_file = discord.File(
                        "assets/video.png", filename="video.png"
                    )
                    thumbnail_url = "attachment://video.png"
                elif "audio" in mime_type:
                    color = discord.Color.blue()
                    thumbnail_file = discord.File(
                        "assets/audio.png", filename="audio.png"
                    )
                    thumbnail_url = "attachment://audio.png"
                ...
                base_embed.set_footer(
                    text="Powered by FFprobe",
                    icon_url="https://img.icons8.com/?size=100&id=32418&format=png&color=000000",
                )
                if thumbnail_url:
    ...
    async def download_media(self, ctx: commands.Context, url: str) -> str:
        if self.is_valid_url(url):
            ...
            file_path = f"{os.path.join(tempfile.gettempdir())}/{file_name}"
            ...
            attachment = (
                ctx.message.attachments[0] or ctx.interaction.message.attachments[0]
            )
            file_name = attachment.filename
            file_path = f"{os.getenv('TEMP', '/tmp')}/{file_name}"
        ...
        try:
            ...
            if "format" in metadata:
                for key, value in metadata["format"].items():
                    if key == "size":
                        flat_metadata["File Size"] = (
                            f"{int(value)} bytes ({self.human_readable_size(int(value))})"
                        )
                    elif key == "duration":
                        duration_seconds = float(value)
                        flat_metadata["Total Duration"] = (
                            f"{duration_seconds} seconds ({self.format_duration(duration_seconds)})"
                        )
                    elif key == "tags":
            ...
            if "streams" in metadata:
                for idx, stream in enumerate(metadata["streams"]):
                    ...
                    flat_metadata[f"Stream {idx + 1} Codec"] = (
                        f"{codec_name} ({codec_tag_string}, {codec_long_name})"
                    )
                    for key, value in stream.items():
                        if key == "duration":
                            duration_seconds = float(value)
                            flat_metadata[f"Stream {idx + 1} Duration"] = (
                                f"{duration_seconds} seconds ({self.format_duration(duration_seconds)})"
                            )
                        elif key == "tags":
                            for tag_key, tag_value in value.items():
                                flat_metadata[
                                    f"Stream {idx + 1} Tag {tag_key.capitalize()}"
                                ] = tag_value
                        elif key not in [
                            "codec_name",
                            "codec_long_name",
                            ...
                            flat_metadata[f"Stream {idx + 1} {key.capitalize()}"] = (
                                value
                            )
